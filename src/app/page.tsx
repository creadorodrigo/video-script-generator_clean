            ...
            {results.roteiros_gerados.map((roteiro) => (
              <div key={roteiro.id} className="bg-white rounded-lg shadow p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold">ROTEIRO #{roteiro.numero} - {roteiro.titulo}</h3>
                    <div className="flex gap-4 text-sm text-gray-600 mt-1">
                      <span>⏱️ {roteiro.duracao_estimada_segundos}s</span>
                      <span>📱 {roteiro.plataformas_recomendadas?.join(', ')}</span>
                    </div>
                  </div>
                  <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-bold">
                    ⭐ {roteiro.score_aderencia}
                  </span>
                </div>
                ...
                <div className="space-y-3 mb-4">
                  <div>
                    ...
                    <p className="mt-2 p-3 bg-blue-50 rounded text-gray-800">{roteiro.gancho?.texto}</p>
                  </div>
                  <div>
                    <span className="text-xs font-bold text-purple-600 bg-purple-50 px-2 py-1 rounded">
                      📝 CORPO ({roteiro.corpo?.timing})
                    </span>
                    <p className="mt-2 p-3 bg-purple-50 rounded text-gray-800 whitespace-pre-line">{roteiro.corpo?.texto}</p>
                  </div>
                  <div>
                    <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded">
                      🎯 CTA ({roteiro.cta?.timing})
                    </span>
                    <p className="mt-2 p-3 bg-orange-50 rounded text-gray-800">{roteiro.cta?.texto}</p>
                  </div>
                </div>
